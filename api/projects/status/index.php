...
if(!$status_page = Input::get('status')){
	echo json_encode(array("message"=>"You need to provide a status first."));
}
else {

	$prf = new Project();
	$obj = new Project();
	$obid = ($obj->find('status_table', $status_page, 'status')) ? $obj->data()->id : "-1";

	$prfr = $prf->find_all('project_table', 'status_id', $obid);
